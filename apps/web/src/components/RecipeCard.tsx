...
function useFontLinks(fonts: Fonts) {
  useEffect(() => {
    ...
    const links = fams.map((f) => fontUrl(f)).filter(Boolean) as string[]
...
function accentOf(recipe: Recipe): string {
  ...
  const v = key ? recipe.tokens?.[key] : undefined
  if (v && /^#[0-9a-fA-F]{3,8}$/.test(v)) return v
  return recipe.themeColor || recipe.background || '#6ee7b7'
}
...
function Swatch({ label, value }: { label: string; value?: string | null }) {
  ...
  return (
    <div className="swatch">
      <span className="chip" style={{ background: v, boxShadow: value ? `0 0 0 1px rgba(255,255,255,0.14), 0 0 14px ${v}33` : undefined }} />
...
function FontRow({ role, family }: { role: string; family?: string | null }) {
  return (
    <div className="fitem">
      ...
      <span className="demo" style={{ fontFamily: family ? `'${family}', sans-serif` : undefined }}>
        {family || '—'}
      </span>
      <span className="v">{family || 'system fallback'}</span>
    </div>
  )
}
...
export function RecipeCard({ recipe, sessionId }: { recipe: Recipe; sessionId: string }) {
  ...
  const tokenEntries = Object.entries(recipe.tokens || {}).filter(([k, v]) => /^#[0-9a-fA-F]{3,8}$/.test(v)).slice(0, 16)
    ...
    <>
      <div className="card">
        <div className="card-head">
          <h2>Recipe</h2>
          {sessionId && <span className="pill mono">{sessionId.slice(0, 8)}…</span>}
        </div>
        <div className="nm-title">{recipe.name}</div>
        <div className="swatches">
          <Swatch label="background" value={recipe.background} />
          <Swatch label="accent" value={accent} />
          <Swatch label="theme-color" value={recipe.themeColor} />
        </div>
        <div className="kv">
          ...
          <div>
            {navCount > 0 ? `${navCount} links${recipe.nav?.transparent ? ' · transparent' : ''}` : '—'}
            {recipe.nav?.position && ` · ${recipe.nav.position}`}
          </div>
          <div className="k">media</div>
          <div>{recipe.images?.length ? `${recipe.images.length} image(s)` : '—'}</div>
          <div className="k">socials</div>
          <div>{recipe.socials?.length ? recipe.socials.map((s) => s.label).join(', ') : '—'}</div>
        </div>
      ...
      <div className="card">
        <h2>Fonts</h2>
        <div className="fonts">
          <FontRow role="display" family={recipe.fonts.display} />
          <FontRow role="body" family={recipe.fonts.body} />
          <FontRow role="mono" family={recipe.fonts.mono} />
        </div>
      </div>
      ...
      {tokenEntries.length > 0 && (
        <div className="card">
          <div className="card-head">
            <h2>CSS tokens</h2>
            <button className="small" onClick={() => setShowTokens((s) => !s)}>
          ...
          {showTokens && (
            <div className="tokens">
              {tokenEntries.map(([k, v]) => (
                <div key={k} className="tok">
                  <span className="k">{k}</span>
                  <span className="chip sm" style={{ background: v }} />
